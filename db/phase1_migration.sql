...
-- ============================================================
-- 8. Create admin_roles table with proper role enum
-- ============================================================
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM app_admins WHERE app_admins.user_id = auth.uid()) THEN
...
--     (safe: only inserts if user exists and isn't already admin)
-- ============================================================

INSERT INTO app_admins (user_id, role)
SELECT au.id, 'super_admin'::admin_role
FROM auth.users au
WHERE au.email = 'gowtham.aidata@gmail.com'
  AND NOT EXISTS (
    SELECT 1 FROM app_admins aa WHERE aa.user_id = au.id
  );

-- Update existing entry to super_admin if already present
UPDATE app_admins
SET role = 'super_admin'::admin_role
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'gowtham.aidata@gmail.com')
...
BEGIN
  ...
  DELETE FROM app_admins WHERE user_id = uid;
