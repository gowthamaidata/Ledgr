...
import {
  ...
  Shield, Plus, Pencil, Trash2, Sun, Moon, Monitor, Save, Loader2, LogOut,
} from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)', marginRight: '12px' }}>
        {account.opening_balance != null ? formatINR(account.opening_balance) : ''}
      </div>
      {hovered && (
...
export default function Settings() {
  ...
  function openEditAccount(account) {
    setAccountForm({ name: account.name, type: account.type, balance: String(account.opening_balance || '') });
    ...
    try {
      if (accountModal === 'add') {
        const { error } = await supabase.from('accounts').insert({
          ...
          opening_balance: parseFloat(accountForm.balance) || 0,
        });
        if (error) throw error;
    ...
    <div style={{ padding: '20px', maxWidth: '640px', margin: '0 auto' }}>
      ...
      <div style={sectionStyle}>
        <Card>
          <CardHeader
            title="Session"
            action={<LogOut size={18} style={{ color: 'var(--text-muted)' }} />}
          />
          <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: '0 0 16px 0' }}>
          ...
          <Button
            ...
            icon={LogOut}
            onClick={async () => {
              const ok = await toast.confirm('Sign out of your account?')
              if (ok) signOut()
            }}
          >
            Sign Out
